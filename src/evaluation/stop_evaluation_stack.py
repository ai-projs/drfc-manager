...
def stop_evaluation_stack(data: Dict[str, Any]):
    ...
    if not stack_name:
        ...
        print(f"Stack name not in data, reconstructing: {stack_name}")
        data['stack_name'] = stack_name

    print(f"Stopping evaluation stack: {stack_name}")
    ...
    try:
        ...
        if docker_style == "swarm":
            ...
            if not eval_compose_paths or not eval_compose_paths[0]:
                 print(f"Warning/Error: Could not resolve path for {ComposeFileType.EVAL.value}. Cannot perform compose down accurately.")
            ...
            if not base_compose_file_path or not os.path.exists(base_compose_file_path):
                 print(f"Warning/Error: Evaluation compose file path not found or invalid: '{base_compose_file_path}'. Cannot perform compose down accurately.")
        ...
        print(f"Error stopping evaluation stack: {e}")
        data["status"] = "error"
        data["error"] = str(e)
