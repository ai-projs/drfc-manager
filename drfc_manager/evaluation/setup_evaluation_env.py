...
def setup_evaluation_env(data: Dict[str, Any]):
    ...
    if not config:
        raise ValueError(
            "setup_evaluation_env requires 'config' in the input data dictionary."
        )

    logger.info(
        f"Setting up environment for evaluation run_id: {config.run_id}, model: {config.model_name}"
    )
    ...
    config.env_vars.DR_SIMTRACE_S3_PREFIX = (
        f"{config.model_name}/evaluation-{eval_time}"
    )
