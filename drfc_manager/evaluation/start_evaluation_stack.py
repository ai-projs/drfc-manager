...
def start_evaluation_stack(data: Dict[str, Any]):
    ...
    if not config:
        raise ValueError(
            "start_evaluation_stack requires 'config' in the input data dictionary."
        )
    ...
    try:
        ...
        if docker_style == "swarm":
            ...
            if services:
                raise DockerError(
                    f"Stack {stack_name} already running (found services). Stop evaluation first."
                )
        ...
        if (
            ...
            logger.info(
                f"Attempting to revert environment prefix to {original_prefix} after failure."
            )
