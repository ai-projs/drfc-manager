...
try:
    ...
    if isinstance(parsed_containers, list) and all(
        ...
        logger.info(
            f"Successfully loaded {len(containers)} container IDs from DR_VIEWER_CONTAINERS."
        )
        logger.debug(f"Loaded container IDs: {containers}")
    else:
        logger.warning(
            f"DR_VIEWER_CONTAINERS was not a list of strings: '{containers_str}'. Treating as empty list."
        )
except json.JSONDecodeError:
    logger.error(
        f"Failed to parse DR_VIEWER_CONTAINERS JSON: '{containers_str}'. Treating as empty list."
    )
except Exception as e:
...
async def health():
    ...
    try:
        ...
        error_details["socket"] = (
            f"Timeout connecting to {target_host}:{target_port} after {HEALTH_CHECK_SOCKET_TIMEOUT}s"
        )
        logger.warning(
            f"Health check: Socket connection to {target_host}:{target_port} timed out."
        )
        ...
        logger.warning(
            f"Health check: Connection refused by {target_host}:{target_port}."
        )
    except Exception as e:
        socket_status = f"error: {type(e).__name__}"
        error_details["socket"] = str(e)
        logger.warning(
            f"Health check: Socket connection test to {target_host}:{target_port} failed: {e}",
            exc_info=True,
        )
        ...
        try:
            async with httpx.AsyncClient(
                timeout=httpx.Timeout(
                    HEALTH_CHECK_PING_TIMEOUT, connect=HEALTH_CHECK_SOCKET_TIMEOUT
                )
            ) as client:
                ...
                if 200 <= resp.status_code < 500:
                    ...
                    error_details["ping"] = (
                        f"Target responded with server error status: {resp.status_code}"
                    )
                    logger.warning(
                        f"Health check: Target {target_ping_url} responded with status {resp.status_code}"
                    )

        except httpx.TimeoutException:
            ping_status = "error: timeout"
            error_details["ping"] = (
                f"Timeout connecting or reading from {target_ping_url} after {HEALTH_CHECK_PING_TIMEOUT}s"
            )
            ...
            logger.warning(
                f"Health check: HTTP connection refused by {target_ping_url}."
            )
        except httpx.RequestError as e:
            ping_status = f"error: {type(e).__name__}"
            error_details["ping"] = (
                f"HTTP request error for {target_ping_url}: {str(e)}"
            )
            logger.warning(
                f"Health check: HTTP request error for {target_ping_url}: {e}"
            )
        except Exception as e:
            ping_status = f"error: {type(e).__name__}"
            error_details["ping"] = (
                f"Unexpected error during HTTP ping to {target_ping_url}: {str(e)}"
            )
            logger.warning(
                f"Health check: Unexpected error during HTTP ping to {target_ping_url}: {e}",
                exc_info=True,
            )
    else:
...
if __name__ == "__main__":
    ...
    if containers:
        ...
        logger.info(
            "No specific container IDs loaded (DR_VIEWER_CONTAINERS not set or empty/invalid). Proxying requests for any container ID."
        )
