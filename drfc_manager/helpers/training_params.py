...
def _setting_envs(train_time: str, model_name: str) -> Dict[str, Any]:
    ...
    if metrics_prefix is not None:
        config["METRICS_S3_OBJECT_KEY"] = "{}/TrainingMetrics.json".format(
            metrics_prefix
        )
    else:
        config["METRICS_S3_OBJECT_KEY"] = (
            "DeepRacer-Metrics/TrainingMetrics-{}.json".format(train_time)
        )

    config["MODEL_METADATA_FILE_S3_KEY"] = os.environ.get(
        "DR_LOCAL_S3_MODEL_METADATA_KEY", "custom_files/model_metadata.json"
    )
    config["REWARD_FILE_S3_KEY"] = os.environ.get(
    ...
    config["CHANGE_START_POSITION"] = os.environ.get(
        "DR_TRAIN_CHANGE_START_POSITION",
        os.environ.get("DR_CHANGE_START_POSITION", "true"),
    )
    config["ROUND_ROBIN_ADVANCE_DIST"] = os.environ.get(
        "DR_TRAIN_ROUND_ROBIN_ADVANCE_DIST", "0.05"
    )
    config["START_POSITION_OFFSET"] = os.environ.get(
        "DR_TRAIN_START_POSITION_OFFSET", "0.00"
    )
    config["ENABLE_DOMAIN_RANDOMIZATION"] = os.environ.get(
    ...
    config["BEST_MODEL_METRIC"] = os.environ.get(
